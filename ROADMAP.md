...
6. **Injeksi EXIF Fisik Pasca-Kompresi (`piexifjs`)**: Mengekstrak kode biner EXIF dari foto asli, lalu disuntikkan secara paksa ke foto hasil autokompresi web (Canvas). Tujuannya agar pengunjung yang mengunduh file hasil kompresi tetap mendapatkan metadata kamera yang utuh (menambah sedikit beban client-side).
7. **Toggle Privasi EXIF per Foto**: Penambahan kolom `hide_exif` (boolean, default: false) di database dan opsi centang di halaman Upload/Edit. Jika diaktifkan, metadata tetap tersimpan di database admin, namun di halaman publik akan diganti dengan pesan elegan *"Metadata disembunyikan oleh kreator"*. Jika foto memang tidak memiliki EXIF dari awal, peringatan *"Exif tidak tersedia"* tetap dipertahankan.