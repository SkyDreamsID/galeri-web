...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (post.story) {
    ...
    // Hitung sisa ruang untuk story agar total karakter tidak lebih dari ~160
    const maxStoryLength = 160 - footerText.length
    const truncatedStory = plainStory.length > maxStoryLength 
      ? plainStory.substring(0, maxStoryLength - 3) + '...' 
      : plainStory
      
    desc = `${truncatedStory}${footerText}`
  } else {
    // Jika tidak ada story, cukup tampilkan pembuatnya tanpa newline berlebih
    desc = `Karya oleh ${creators}`
  }
...
export default async function PostDetail({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <>
      ...
      <main className="container mx-auto max-w-7xl px-4 md:px-8 py-10 md:py-16">
        ...
        <div className="flex flex-col max-lg:landscape:grid max-lg:landscape:grid-cols-11 lg:grid lg:grid-cols-11 max-lg:landscape:gap-6 lg:gap-16 items-start">
          ...
          <div className="max-lg:landscape:col-span-5 lg:col-span-6 order-1 w-full max-lg:landscape:sticky max-lg:landscape:top-24 lg:sticky lg:top-24 self-start">
            <div className="relative">
              ...
              <EmblaCarousel photos={photos} postId={postData.id} hideExif={postData.hide_exif} postTitle={postData.title} />
            </div>
          </div>
